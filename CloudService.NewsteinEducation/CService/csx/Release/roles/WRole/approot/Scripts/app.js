...
(function () {
    ...
    ]).controller("studentChartCtrl", ["$scope",
        function ($scope) {
            return $scope.mainData = [{
                student: "Algebra",
                Report1: 70,
                    ...
                    switch (data = scope.data, attrs.type) {
                        ...
                        case "bar":
                            return colors = void 0 === attrs.barColors || "" === attrs.barColors ? null : JSON.parse(attrs.barColors), options = {
                                ...
                                ykeys: JSON.parse(attrs.ykeys),
                                labels: JSON.parse(attrs.labels),
                                barColors: colors || ["#df4a32", "#fdd761", "#a3c86d", "#afd8f8", "#edc240", "#cb4b4b", "#9440ed"],
    ...
    angular.module("app", ["ngRoute", "ngAnimate", "ui.bootstrap", "easypiechart", "mgo-angular-wizard", "app.ui.ctrls", "app.ui.services", "app.controllers", "app.directives", "app.form.validation", "app.ui.form.ctrls", "app.ui.form.directives", "app.tables", "app.task", "app.localization", "app.chart.ctrls", "app.chart.directives"]).config(["$routeProvider",
        function ($routeProvider) {
            return $routeProvider.when("/", {
                ...
                templateUrl: "views/student_dashboard.html"
            }).when("/ui/typography", {
            ...
            }).when("/pages/tutor_profile", {
                templateUrl: "views/pages/tutor_profile.html"
            }).when("/404", {
            ...
            controller: ["$scope", "$element", "$location",
                function ($scope, $element, $location) {
                    ...
                    return path = function () {
                        return $location.absUrl().toLowerCase()
                    }, addBg = function (path) {

                        var specific = $location.absUrl().toLowerCase();
                        specific = (specific.indexOf("/error404") != -1 || specific.indexOf("/account/login") != -1 ||
                    specific.indexOf("/error500") != -1 || specific.indexOf("/pages/signin") != -1 ||
                    specific.indexOf("/account/register") != -1 || specific.indexOf("/pages/signup") != -1
                    );
                        var tasks = $location.absUrl().toLowerCase();
                        tasks = (tasks.indexOf("/tasks") != -1)

                        if (specific)
                            return $element.addClass("body-special");
                        else if (tasks)
                            return $element.addClass("body-tasks");
                        else if (($element.removeClass("body-home body-special body-tasks"), path) == "/")
    ...
    angular.module("app.controllers", []).controller("AppCtrl", ["$scope", "$location",
     function ($scope, $location) {
         return $scope.isSpecificPage = function () {
             ...
             var specific = $location.absUrl().toLowerCase();
             return (specific.indexOf("/error404") != -1 || specific.indexOf("/account/login") != -1 ||
                 specific.indexOf("/error500") != -1 || specific.indexOf("/pages/signin") != -1 ||
                 specific.indexOf("/account/register") != -1 || specific.indexOf("/pages/signup") != -1
                 );
